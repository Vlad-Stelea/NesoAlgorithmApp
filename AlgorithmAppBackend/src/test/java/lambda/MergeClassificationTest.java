...
public class MergeClassificationTest {
    ...
    public void setup() {
        ...
        mcRequest = new MergeClassificationRequest(child1Name, child2Name,newName,parentName);
    }
    ...
    public void testReclassifyAlgorithm() throws SQLException {
        ...
        assertEquals(handleResult.response, "Success ,parent_class");
        assertEquals(handleResult.httpCode, 200);
    }

    @Test
    public void testFailReclassifyAlgorithm() throws SQLException {
        when(dao.mergeClassification(child1Name, parentName)).thenReturn(false);
        MergeClassificationResponse handleResult = mcHandler.handle(mcRequest);
        assertEquals(handleResult.response, "Success (parent_class)");
